...
public:
    ...
    bool full(Extended_stack&) const ; //  If the stack is full, return true; else return false.
    int size() const;   //  Return the number of entries in the stack.

    Error_code pop_top(Extended_stack&, Stack_entry&);
    Error_code clear(Extended_stack&);
    int size(Extended_stack&) const;
    void delete_all(Extended_stack&, Stack_entry&);
    void prime(Extended_stack&);
