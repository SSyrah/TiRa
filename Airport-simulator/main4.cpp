...
void main_4() //  Airport simulation program
/*
...

{
    ...
    {                                                         //  loop over time intervals
        ...
        for (int i = 0; i < number_arrivals; i++)
        {
            ...
            if (small_airport_landing.get_landing_size() < small_airport_takeoff.get_landing_size())
            {
                if (small_airport_landing.can_land(current_plane) != success)
                {
                    if (small_airport_takeoff.can_land(current_plane) != success)
                    {
        ...
        for (int j = 0; j < number_departures; j++)
        {
            ...
            {
                if (small_airport_takeoff.is_landing_empty() || small_airport_landing.is_landing_empty())
                {
                    small_airport_takeoff.can_depart(current_plane);
                    small_airport_takeoff.activity2(current_time,current_plane);
                }
                else
                {
                    current_plane.refuse();
    ...
    small_airport_takeoff.short_shut_down(end_time);
    small_airport_landing.short_shut_down(end_time);
}
